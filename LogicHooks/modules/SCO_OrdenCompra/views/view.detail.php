...
class SCO_OrdenCompraViewDetail extends ViewDetail {
 	 ...
 	 public function preDisplay()
    {
 	...
 	function display(){
 		$sty = "<style>#alertapp{ position: fixed; float: left; margin-top: 10px; margin-left:35%; z-index:1;}#idpro tbody tr{border-bottom: 1px solid #ccc; background-color: #f2f2f2;} .search-form{display: none;}</style>";
 		$htmlpp ='<div id="alertapp"></div>';
       	echo $sty.$htmlpp;
		...
		htmlpro += '<thead>';
		htmlpro += '	<tr>';
		htmlpro += '<th style=\"width: 10%;\">Cod Pro</th><th style=\"width: 40%;\">Descripcion</th><th style=\"width: 5%;\">Unidad</th><th style=\"width: 5%;\">Cantidad</th><th style=\"width: 5%;\">Prec Uni</th><th style=\"width: 5%;\">Desc %</th><th style=\"width: 5%;\">Desc valor</th><th style=\"width: 5%;\">Sub total</th><th style=\"width: 10%;\">Proy C/O</th>';
		htmlpro += '	</tr>';
		htmlpro += '</thead>';
		htmlpro += '<tbody id=\"valtr\">';
		htmlpro += '</tbody>';
      ...
      c = c.replace(/[*][*]/g,'\"');
	    ...
	    for(var i = 0 ; i < c.length;i++){
          ...
          if(d[j] != ''){
	    		  celda += '<td style=\"text-align: justify;\">'+d[j]+'</td>';
          }else{
            celda += '<td style=\"border:solid 1px #FFF;background:#d9534f;\">'+d[j]+'</td>';
          }
	    	}
	    fila += '<tr><td width=\"1%\" style=\"text-align: center; background: #FFF;color: #000;\">'+(i+1)+'</td>'+celda+'</tr>';
	    }
		...
		htmlpro += '<thead>';
		htmlpro += '	<tr style=\"border-bottom: 1px solid #888;\">';
		htmlpro += '<th style=\"width:1%;\">#</th><th style=\"width: 10%;\">Cod Pro</th><th style=\"width: 40%;\">Descripcion</th><th style=\"width: 7%;\">Unidad</th><th style=\"width: 7%;\">Cantidad</th><th style=\"width: 7%;\">Prec Uni</th><th style=\"width: 7%;\">Desc %</th><th style=\"width: 7%;\">Desc valor</th><th style=\"width: 7%;\">Sub total</th><th style=\"width: 7%;\">Proy C/O</th>';
		htmlpro += '	</tr>';
		htmlpro += '</thead>';
		htmlpro += '<tbody>';
		htmlpro += fila;
		...
		htmlpro += '<tfoot>';
		htmlpro += '<tr>';
		htmlpro += '<td></td><td><b>SubTotal</b></td><td>'+c2[0]+'</td>';
		htmlpro += '</tr>';
		htmlpro += '<tr>';
		htmlpro += '<td></td><td><b>Descuento Valor</b></td><td>'+c2[1]+' </td>';
		htmlpro += '</tr>';
		...
		htmlpro += '</tr>';
		htmlpro += '<tr>';
		htmlpro += '<td></td><td><b>Total</b></td><td>'+c2[3]+'</td>';
		htmlpro += '</tr>';
		htmlpro += '</table>';
      ...
      c = c.replace(/[*][*]/g,'\"');
        ...
        echo "<style>#list_subpanel_sco_ordencompra_sco_productos .list{display:none;}</style>";
 			...
 			case '2':
 				...
 				echo '<div class="yui-navset detailview_tabs yui-navset-top"><div class="yui-content"><div class="detail view  detail508 expanded">
 					...
 					<tr>
		 				...
		 				<td width="28%" >
		 				<input type="button" id="btn-estados" class="btn btn-sm btn-success" style="padding: 2px 5px;" onClick="estado(3);" value="'.$arr_estado[3].'">
						</td>
            <td width="12.5%" scope="col">
		 					<b>Reportes:</b>
		 				</td>
         ...
         #detail_header_action_menu {pointer-events: none; cursor: default;}
               ...
               $id_usuario = $current_user->id;
               if($id_usuario == $this->bean->user_id_c || $id_usuario == '1'){
	      echo '<input type="button" id="btn-estados" class="btn btn-sm btn-danger" style="padding: 0.8px 8px; background-color: #d9534f !important;" onClick="estado(5);" value="Cancelar Orden">';
             }
        echo '
						</td>
						<td width="12.5%" scope="col">
